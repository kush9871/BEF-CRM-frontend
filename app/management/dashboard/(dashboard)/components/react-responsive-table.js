...
const ReactResponsiveTable = ({ columns, data, serialize = false, initialCount = 0, rowStyle  }) => {
    ...
    <Table>
      ...
      <Tbody>
        {data && Array.isArray(data) && data.map((item, index) => (
          <Tr key={`data${index}`} style={(rowStyle ? rowStyle(item, index) : {})}>
            {serialize && <Td>{index + Number(initialCount) + 1}</Td>}
            {columns.map((col, colInd) => (
              <Td key={`data${index}${colInd}`} className='responsive-table-td'>
