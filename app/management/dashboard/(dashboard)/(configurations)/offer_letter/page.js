...
const OfferLetter = () => {
    ...
    const fetchData = async () => {
      try {
        const res = await Axios.get(
          `${Baseurl}/offer-latter/get-all-offer-latter?page=${page}&limit=${limit}`,
          {
            authenticated: true,
          }
        );
  ...
  const handleDeleteAction = async (id) => {
    try {
      await Axios.delete(`${Baseurl}/offer-latter/delete-offer-latter/${id}`, {
        authenticated: true,
      });
      ...
      toast.error(
        error.response.data.message ||
          "An error occurred while deleting Offer Letter"
      );
    }
  };
      ...
      <div className="breadcrumb-wrapper">
        <div>
          ...
          <Breadcrumb>
            ...
            <span className="ms-2">/ Offer Letter</span>
          </Breadcrumb>
        </div>
        <div className="dashboard-add-btn">
          <Link
            href="/management/dashboard/offer_letter/add-new"
