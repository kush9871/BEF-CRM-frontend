...
const Deduction = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await Axios.get(`${Baseurl}/Deductions/get-all-deduction?page=${page}&limit=${limit}`, {
          authenticated: true,
        });
  ...
  const handleDeleteAction = async (id) => {
    try {
      await Axios.delete(`${Baseurl}/Deductions/delete-deduction/${id}`, {
        authenticated: true,
      });
      ...
      toast.error(
        error.response.data.message ||
          "An error occurred while deleting Deductions"
      );
    }
  };
      ...
      <div className="breadcrumb-wrapper">
        <div>
          ...
          <Breadcrumb>
            ...
            <span className="ms-2">/ Deductions</span>
          </Breadcrumb>
        </div>
        <div className="dashboard-add-btn">
          <Link
            href="/management/dashboard/deduction/add-new"
