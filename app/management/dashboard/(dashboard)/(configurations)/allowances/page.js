...
const Allowances = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await Axios.get(`${Baseurl}/Allowances/get-all-allowance?page=${page}&limit=${limit}`, {
          authenticated: true,
        });
  ...
  const handleDeleteAction = async (id) => {
    try {
      await Axios.delete(`${Baseurl}/Allowances/delete-allowance/${id}`, {
        authenticated: true,
      });
      setKpis((prev) => prev.filter((item) => item._id !== id));
      toast.success("Allowance deleted successfully");
    } catch (error) {
      toast.error(error.response.data.message || "An error occurred while deleting allowance");
    }
  };
      ...
      <div className="breadcrumb-wrapper">
        <div>
          ...
          <Breadcrumb>
            ...
            <span className="ms-2">/ Allowances</span>
          </Breadcrumb>
        </div>
        <div className="dashboard-add-btn">
          <Link
            href="/management/dashboard/allowances/add-new"
