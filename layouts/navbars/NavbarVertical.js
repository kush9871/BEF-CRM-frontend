...
const NavbarVertical = (props) => {
	...
	// Assume userRole comes from props or context
const actualRole = session?.user?.role;

const userRole =
  actualRole === 'super-admin' || actualRole === 'admin'
    ? actualRole
    : 'user';
	// Role check function
	const hasAccess = (menuItem) => {
		if (!menuItem.role) return true; // if no role defined, show to all
		return menuItem.role.includes(userRole);
	};

	// Filter menu recursively based on role access
		...
		return menuList
			...
			.map(menu => {
				if (menu.children) {
					return {
						...menu,
						children: filterMenuByRole(menu.children)
					};
				}
				return menu;
			});
	...
	const filteredMenu = filterMenuByRole(DashboardMenu);
		...
		<Fragment>
			<SimpleBar style={{ maxHeight: '100vh' }}>
				<div className="nav-scroller text-center">
					<Link href="/management/dashboard" className="navbar-brand aside-navbar-top">
						<img src="/images/digicaseLogo.png" className="sideBarLogo img-fluid" style={{minHeight:"42px"}} alt="Org-Image"/>
					</Link>
				</div>
				{/* Dashboard Menu */}
				<Accordion defaultActiveKey="0" as="ul" className="navbar-nav flex-column mt-4">
																							...
																							<Accordion
																								...
																								<Accordion.Collapse
																									...
																									<ListGroup
																										...
																										{menuLevel2Item.children.map(function (
																											menuLevel3Item,
																											menuLevel3Index,
																										) {
																											return (
